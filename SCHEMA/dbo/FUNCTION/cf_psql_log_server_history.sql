...
CREATE OR REPLACE FUNCTION dbo.cf_psql_log_server_history(_ip text, _db_name text) RETURNS TABLE(n_xact_commit numeric, n_numbackends numeric, dx_created timestamp with time zone, n_xact_commit_avg numeric, n_numbackends_avg numeric)
    LANGUAGE plpgsql
    AS $$
/**
* @params {text} _ip - IP - адрес
...
*/
DECLARE
	_n_xact_commit_avg numeric;
	_n_numbackends_avg numeric;
BEGIN
	with items as (
		select 
			sum(p.n_xact_commit) as n_xact_commit,
			...
			p.dx_created
		from dbo.cd_psql as p
		where p.c_ip = _ip and p.c_datname = _db_name
		group by p.dx_created
	)
	select avg(t.n_xact_commit), avg(t.n_numbackends) into _n_xact_commit_avg, _n_numbackends_avg 
	from (SELECT 
		sum(d.n_xact_commit) - sum(coalesce(d.n_xact_commit_prev, 0)) as n_xact_commit, 
		avg(d.n_numbackends) as n_numbackends
	FROM items as d
