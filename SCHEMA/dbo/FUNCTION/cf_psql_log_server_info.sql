CREATE OR REPLACE FUNCTION dbo.cf_psql_log_server_info(_ip text) RETURNS TABLE(n_xact_commit numeric, n_numbackends numeric, dx_created timestamp with time zone)
    LANGUAGE plpgsql
    AS $$
/**
...
*
* @example
* [{ "action": "cf_psql_log_server_info", "method": "Select", "data": [{ "params": [_ip] }], "type": "rpc", "tid": 0}]
*/
BEGIN
	RETURN QUERY with items as (
		...
		where p.c_ip = _ip and p.dx_created between now() - interval '1 day' and now()
...
ALTER FUNCTION dbo.cf_psql_log_server_info(_ip text) OWNER TO mobnius;

COMMENT ON FUNCTION dbo.cf_psql_log_server_info(_ip text) IS 'Транзакции';
