CREATE OR REPLACE FUNCTION dbo.cf_net_server_history(_ip text) RETURNS TABLE(n_sent numeric, n_received numeric, dx_created timestamp with time zone)
    LANGUAGE plpgsql
    AS $$
/**
...
BEGIN
	...
	SELECT avg(d.n_sent) as n_sent, avg(d.n_received) as n_received, max(d.dx_created) as dx_created 
	FROM items as d
